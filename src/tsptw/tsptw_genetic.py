...
class TSPTWGenetic:
    ...
    def evaluate_cost(self, dist, wait, late, parameters, depot, subroute, k1, k2):
        ...
        cost = [1.0 + y * wait_val + z * late_val if x == 0 else 1.0 + x * dist_val + y * wait_val + z * late_val for
                x, y, z, dist_val, wait_val, late_val in
                zip(dist, wait, late, dc[subroute_], tw_wc[subroute_], tw_lc[subroute_])]
        return cost
    ...
    def evaluate_cost_penalty(self, dist, time, wait, late, parameters, depot, subroute, route, k1, k2):
        ...
        if (route == 'open'):
            ...
            subroute_ = depot + subroute + depot
        cost = [0] * len(subroute_)
        cost = [1.0 + y * wait_val + z * late_val if x == 0 else cost[0] + x * dist_val + y * wait_val + z * late_val
                for x, y, z, dist_val, wait_val, late_val in
                zip(dist, wait, late, dc[subroute_], tw_wc[subroute_], tw_lc[subroute_])]
    ...
    def show_report(self, solution, distance_matrix, parameters, route):
        ...
        for i in range(0, len(solution[1])):
            ...
            for j in range(0, len(subroute[0])):
                ...
                report_lst.append(
                    ['#' + str(i + 1), activity, subroute[0][j], round(wait[j], 2), arrive_time, round(time[j], 2),
                     round(dist[j], 2), round(late[j], 2)])
        report_lst.append(['-//-', '-//-', '-//-', '-//-', '-//-', '-//-', '-//-', '-//--'])
    ...
    def target_function(self, population, distance_matrix, parameters, route, k1, k2):
        ...
        for k in range(0, len(population)):  # k individuals
            ...
            while (size > i):  # i subroutes
                ...
                wait, time, late = self.evaluate_time(distance_matrix, parameters, depot=individual[0][i],
                                                      subroute=individual[1][i])

                cost_s = self.evaluate_cost(dist, wait, late, parameters, depot=individual[0][i],
                                            subroute=individual[1][i], k1=k1, k2=k2)
    ...
    def crossover_tsp_bcr(self, parent_1, parent_2, distance_matrix, parameters, route, k1, k2):
        ...
        for i in range(0, 2):
            ...
            dist_list = [
                self.evaluate_distance(distance_matrix, insertion[0], insertion[1][:n] + [A] + insertion[1][n:]) for n
                in range(0, len(parent_2[1][0]) + 1)]
            wait_time_list = [
                self.evaluate_time(distance_matrix, parameters, insertion[0], insertion[1][:n] + [A] + insertion[1][n:])
                for n in range(0, len(parent_2[1][0]) + 1)]
            insertion_list = [insertion[1][:n] + [A] + insertion[1][n:] for n in range(0, len(parent_2[1][0]) + 1)]
            d_2_list = [self.evaluate_cost_penalty(dist_list[n], wait_time_list[n][1], wait_time_list[n][0],
                                                   wait_time_list[n][2], parameters, insertion[0], insertion_list[n],
                                                   route, k1, k2) for n in range(0, len(dist_list))]
            d_2 = min(d_2_list)
            if (d_2 <= d_1):
                d_1 = d_2
    ...
    def breeding(self, cost, population, fitness, distance_matrix, elite, parameters, route, k1, k2):
        ...
        for i in range(elite, len(offspring)):
            ...
            if (len(parent_1[1]) == 1 and len(parent_2[1]) == 1):
                if (rand > 0.5):
                    offspring[i] = self.crossover_tsp_bcr(parent_1, parent_2, distance_matrix, parameters=parameters,
                                                          route=route, k1=k1, k2=k2)
                elif (rand <= 0.5):
                    offspring[i] = self.crossover_tsp_bcr(parent_2, parent_1, distance_matrix, parameters=parameters,
                                                          route=route, k1=k1, k2=k2)
        return offspring
    ...
    # GA-VRP Function
    def genetic_algorithm_tsp(self, coordinates, distance_matrix, parameters, population_size=5, route='closed',
                              mutation_rate=0.1, elite=0, generations=50, k1=10, k2=100):
        count = 0
