...
class PyVRPSolver:
    ...
    def solve(self, launch_count, data_dir='cluster_result/'):
        ...
        for i in range(launch_count):
            ...
            # Call GA Function
            ga_report, ga_vrp = genetic_algorithm_vrp(coordinates, distance_matrix, parameters, self.velocity, self.fixed_cost,
                                                      self.variable_cost, self.capacity, self.population_size, self.vehicle_types, self.n_depots,
                                                      self.route, self.model, self.time_window, self.fleet_size, self.mutation_rate, self.elite,
                                                      self.generations, self.penalty_value, self.graph)

            plot_data = {'coordinates': coordinates, 'ga_vrp': ga_vrp, 'n_depots': self.n_depots, 'route': self.route}
